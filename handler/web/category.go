...
func (c *categoryWeb) Category(ctx *gin.Context) {
	...
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, model.ErrorResponse{Error: "GetSession by email!" + err.Error()})
		return
	}
	...
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, model.ErrorResponse{Error: "Tempalte new!" + err.Error()})
		return
	}

	err = t.Execute(ctx.Writer, dataTemplate)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, model.ErrorResponse{Error: "Template exectue!" + err.Error()})
	}
}
